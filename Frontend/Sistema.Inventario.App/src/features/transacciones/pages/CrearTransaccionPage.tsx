...
export default function CrearTransaccionPage() {
  ...
  const [formulario, setFormulario] = useState<CrearTransaccionRequest>({
    tipoTransaccion: '',
    productoId: '',
    cantidad: 1,
    precioUnitario: 0,
  ...
  const handleCambio = (campo: keyof CrearTransaccionRequest, valor: string | number) => {
  ...
  const handleSeleccionarProducto = (productoId: string) => {
    ...
    setErrores((prev) => ({ ...prev, productoId: undefined, precioUnitario: undefined }));
  };
    ...
    <div className="pagina">
      ...
      <form className="formulario" onSubmit={handleSubmit}>
        ...
        <div className="formulario-acciones">
          ...
          <Button type="submit" disabled={enviando}>{enviando ? 'Guardando...' : 'Crear Transacción'}</Button>
        </div>
      </form>
    </div>
  );
